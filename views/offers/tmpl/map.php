...
foreach ( $offers as $offer ) {
	...
	$contentString = '<div class="info-box">' . '<div class="title">' . htmlspecialchars($offer->subject) . '</div>' . '<div class="info-box-content">' . '<div class="address" itemtype="http://schema.org/PostalAddress" itemscope="" itemprop="address">' .  htmlspecialchars(JBusinessUtil::getAddressText ($offer)) . '</div>';
	if (!empty($offer->phone)) {
		$contentString .= '<div class="info-phone"><i class="dir-icon-phone"></i> ' . $db->escape($offer->phone) . '</div>';
	}

	$contentString .= '<a href="' . $db->escape(JBusinessUtil::getOfferLink ($offer->id, $offer->alias)) . '"><i class="dir-icon-external-link"></i> ' . $db->escape(JText::_("LNG_MORE_INFO", true)) . '</a>' . '</div>' . '<div class="info-box-image">' . (!empty($offer->picture_path) ? '<img src="' . JURI::root() . PICTURES_PATH . $offer->picture_path . '" alt="' . htmlspecialchars($offer->subject) . '">' : "") . '</div>' . '</div>';
...
<script>
    jbdMap.construct(<?php echo json_encode($offer_locations) ?>, <?php echo json_encode($initparams) ?>, <?php echo $appSettings->map_type ?>);
    var mapInstance = jbdMap.loadMapScript();

    function initializeOfferSearchAutocomplete(token, preventSubmit) {
