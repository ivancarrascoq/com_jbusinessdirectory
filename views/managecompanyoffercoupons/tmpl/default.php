...
<form action="<?php echo JRoute::_('index.php?option=com_jbusinessdirectory&view=managecompanyoffercoupons');?>" method="post" name="adminForm" id="adminForm">
	<div id="editcell">
		<table class="dir-table dir-panel-table responsive-simple">
			<thead>
				<tr>
				...
				if(!empty($this->items)){
					foreach($this->items as $coupon) { ?>
						<tr class="row<?php echo $nrcrt%2 ?>">
							...
							<td data-title="<?php echo JText::_('LNG_OFFER'); ?>" class="has-title">
								<?php echo $coupon->offer; ?>
							</td>
							<td data-title="<?php echo JText::_('LNG_COMPANY'); ?>" class="has-title">
								<?php echo $coupon->company; ?>
							</td>
							<td data-title="<?php echo JText::_('LNG_GENERATED_TIME'); ?>" class="has-title">
								<?php echo JBusinessUtil::getDateGeneralFormat($coupon->generated_time); ?>
							</td>
							<td data-title="<?php echo JText::_('LNG_EXPIRATION_TIME'); ?>" class="has-title">
