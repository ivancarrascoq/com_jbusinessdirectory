				...
				if (isset($packageFeatures) && in_array($attribute->code, $packageFeatures) || !$enablePackages)
				{
					...
					switch ($attribute->attributeTypeCode)
					{
						...
						case "radio":
							...
							foreach ($attributeOptions as $key => $option)
							{
							    $renderedContent .= "<div class='custom-attr-radio'>";
								$option = "<span class='option'>" . $option . "</span>";
		...
		if(!empty($attributes)){
			...
			foreach($attributes as $attribute) {
				...
				if(isset($packageFeatures) && in_array($attribute->code,$packageFeatures) || !$enablePackages){
					...
					switch ($attribute->attributeTypeCode){
						case "header":
							$renderedContent.="<h3 class='attribute-header attribute-name'>".$attribute->name."</h3>";
							break;
						case "input":
							$inputValue= $attribute->attributeValue;
							if(!empty($inputValue)){
								$renderedContent.='<ul class="business-properties  attribute-'.$attribute->id.'">';
								$renderedContent.= '<li class="attribute-name"><div>'.$attribute->name.': </div></li>';
								$renderedContent.= '<li class="attribute-value">'.$inputValue.'</li>';
								$renderedContent.= '</ul>';
							}
							break;
						case "link":
							...
							if(!empty($inputValue)){
								...
								$renderedContent.='<ul class="business-properties  attribute-'.$attribute->id.'">';
								$renderedContent.= '<li class="attribute-name"><div>'.$attribute->name.': </div></li>';
								$renderedContent.= '<li class="attribute-value"> <a target="_blank" href="'.$inputValue.'">'.$inputValue.'</a></li>';
								$renderedContent.= '</ul>';
							}
							break;
						case "textarea":
						...
						case "checkbox":
							...
							foreach ($attributeOptions as $key=>$option){
								if( in_array($attributeOptionsIDS[$key] , $attributeValues)){
								    $renderedContent.= '<li class="attribute-value">';
