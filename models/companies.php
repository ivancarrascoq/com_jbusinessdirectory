...
{ 
    ...
    function getCompany($cmpId=null) {
        ...
        }else if(!empty($this->appSettings->max_categories)){
	...
	function contactCompany($data){
		...
		$company->load(intval($data['companyId']));
        ...
        $tmp = array();
        $tmp["id"] = 0;
        $tmp["item_id"] = intval($data['companyId']);
	...
	function checkBusinessAboutToExpire(){
		...
		foreach($companies as $company){
			echo "sending expiration e-mail to: ".$company->name."<br/>";
	 ...
	 * @return mixed
	 */
	function increaseStatisticCount($companyId,$typeId,$articleId = null){
		
		$company = $this->getCompany();
		
		$companiesTable = $this->getTable("Company");

		if ($typeId == STATISTIC_TYPE_WEBSITE_CLICK) {
