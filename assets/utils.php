...
class JBusinessUtil{
	...
	private static function getAppSettings(){
		...
		$appSettings->url_fields = explode(",",$appSettings->url_fields);
	 ...
	 * Exception: the item descriptions
	 * 
	 */
	...
	static function getDateISOFormat($data){
	    $dateS="";
	    if(isset($data) && strlen($data)>6  && $data!="0000-00-00"){
	        //$data =strtotime($data);
	        //setlocale(LC_ALL, 'de_DE');
	...
	static function getShortWeekDate($data){
	    if(empty($data)){
	        return null;
	    }
	    
	    $date = JFactory::getDate($data);
	    $dateS = $date->format('l, M j');
	...
	/**
	 * Process listing packages to estabils the state and expiration date
	 * 
	 */
	public static function processPackages($items){
	    ...
	    foreach($items as $item){
	        ...
	        foreach($orders as $i=>$orderS){
	            ...
	            $packgeInfo->active = ($package->price==0 || $package->expiration_type==1 || (time() > strtotime($order->start_date) and (time() < strtotime( $order->start_date."+  $package->days day")) and $order->start_date!='0000-00-00')) ;
	            
	            if($packgeInfo->active){
	                $active = true;
	            }
	            $packgeInfo->state = $order->state;
	            $packgeInfo->expirationDate = date("Y-m-d",strtotime($order->start_date."+  $package->days day"));
	            $packgeInfo->start_date = $order->start_date;
	            $packgeInfo->future = (time() < strtotime($order->start_date) && $order->start_date!='0000-00-00');
	            
	            if (!$packgeInfo->active && (count($packages) ==1 || $i==(count($packages) - 1)) || ($packgeInfo->active || $packgeInfo->future)) {
	...
	**/
	 ...
	 * @return string
	 */
	public static function getDirectionURL($origin, $destination, $mapType = null) {
		$db = JFactory::getDbo();
		...
		else if ($mapType == MAP_TYPE_BING) {
			if (!empty($origin) && isset($origin["latitude"]) && isset($origin["longitude"])) {
				$source = "pos." . $db->escape($origin["latitude"]) . "_" . $db->escape($origin["longitude"]);
			}

			if (!empty($destination) && isset($destination->latitude) && isset($destination->longitude)) {
				$source = "pos." . $db->escape($destination->latitude) . "_" . $db->escape($destination->longitude);
			}
		...
		else if ($mapType == MAP_TYPE_OSM) {
			if (!empty($origin) && isset($origin["latitude"]) && isset($origin["longitude"])) {
				$source = "loc=" . $db->escape($origin["latitude"]) . "%2C" . $db->escape($origin["longitude"]);
			}

			if (!empty($destination) && isset($destination->latitude) && isset($destination->longitude)) {
				$source = "loc=" . $db->escape($destination->latitude) . "%2C" . $db->escape($destination->longitude);
			}
    ...
    public static function filterPaymentMethods($order, $paymentMethods){
        ...
        if($order->package->expiration_type == 3 || $order->package->expiration_type == 4){
            foreach($paymentMethods as $paymentMethod){
                if(isset($paymentMethod->recurring)){
	...
	public static function loadMapScripts($mapType = null, $asyncDefer = false) {
		...
		if ($mapType == MAP_TYPE_GOOGLE) {
			...
			JHtml::_('script', "https://maps.googleapis.com/maps/api/js?language=" . $lang . $key . "&libraries=geometry&libraries=places");
			JHtml::_('script', 'components/com_jbusinessdirectory/assets/js/google-map.js');
		}
		else if ($mapType == MAP_TYPE_BING) {
			...
			if ($asyncDefer) {
				echo '<script src="https://www.bing.com/api/maps/mapcontrol?callback=loadMapCallback" async defer />';
			} else {
				JHtml::_('script', "https://www.bing.com/api/maps/mapcontrol?callback=loadMapCallback");
			}
			JHtml::_('script', 'components/com_jbusinessdirectory/assets/js/bing-map.js');
		}
		else if ($mapType == MAP_TYPE_OSM) {
			JHtml::_('stylesheet', "components/com_jbusinessdirectory/assets/css/leaflet/leaflet.css");
			JHtml::_('stylesheet', "components/com_jbusinessdirectory/assets/css/leaflet/leaflet-search.css");
			JHtml::_('script', "components/com_jbusinessdirectory/assets/js/leaflet/leaflet.js");
			JHtml::_('script', "components/com_jbusinessdirectory/assets/js/leaflet/leaflet-search.js");
			JHtml::_('script', 'components/com_jbusinessdirectory/assets/js/open-map.js');

			if ($appSettings->enable_map_clustering) {
				JHtml::_('script', 'components/com_jbusinessdirectory/assets/js/leaflet/leaflet.markercluster.js');
				JHtml::_('stylesheet', 'components/com_jbusinessdirectory/assets/css/leaflet/MarkerCluster.css');
				JHtml::_('stylesheet', 'components/com_jbusinessdirectory/assets/css/leaflet/MarkerCluster.Default.css');
			}
		}
	 ...
	 * @since 4.9.5
	 */
	public static function getStaticMap($lat, $long, $class = "company map", $width = 300, $height = 300, $zoom = 13, $mapType = null) {
		$db = JFactory::getDbo();
		$appSettings = JBusinessUtil::getInstance()->getApplicationSettings();
		...
		if ($mapType == MAP_TYPE_GOOGLE) {
			...
			$src = JURI::root().'components/'.JBusinessUtil::getComponentName().'/libraries/staticmaplite/staticmap.php?';
			$src .= "center=".$lat.",".$long;
			$src .= "&zoom=".$zoom;
    ...
    static function getMoreSearchFilterOptions(){
        $options = array();
