...
class JBusinessDirectoryControllerCompanies extends JControllerLegacy
{
	...
	function showCompanyWebsite(){
		$model = $this->getModel('companies');
		$company = $model->increaseStatisticCount(JRequest::getVar('companyId'),STATISTIC_TYPE_WEBSITE_CLICK);
		
		$this->setRedirect($company->website);
	}

	function increaseStatisticCount(){
		$model = $this->getModel('companies');
		$company = $model->increaseStatisticCount(JRequest::getVar('companyId'),JRequest::getVar('type'),JRequest::getVar('articleId'));
